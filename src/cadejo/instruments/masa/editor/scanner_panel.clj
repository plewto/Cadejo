...
;;
(def lin-rate (math/linear-function 0 min-scanner-rate 1 max-scanner-rate))
(def inv-lin-rate (math/linear-function min-scanner-rate 0 max-scanner-rate 1))
...
(defn scanner-panel [drw ied p0]
  (let [data (fn [param] ;; Returns current program value of parameter
    ...
    (label drw [(+ x2 65)(+ y2 40)] "Mod Spread")
    (label drw [(+ x0 8) (+ y2 68)] "Cross Mix")
    (label drw [(+ x0 8) (+ y2 118)] "Efx Mix")
    (text/text (.root drw) [(+ x0 215)(+ y1 20)] "Scanner"
               :style :snaes
               :size 8
               :color (lnf/text-color))
    ...
    (let [y1 (+ y2 65)
          ...
          xb (math/mean xa xc)
          xd (math/mean xc xe)
          cmap {xa, (lnf/major-tick-color)
                xc, (lnf/major-tick-color)
                xe, (lnf/major-tick-color)}]
      (doseq [x [xa xb xc xd xe]]
        (let [c (get cmap x (lnf/minor-tick-color))]
          (tick drw [x y2] c)
          (tick drw [x y1] c))))
