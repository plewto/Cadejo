...
(defn overview-editor [performance ied]
  (let [op-count 8
        ...
        syncfn (fn [data]
                 (if (pos? (count data))  ;; BUG 0005 hack
                   (let [freqlst [(:op1-detune data)(:op2-detune data)
                     ...
                     (dotimes [op op-count]
                       (let [f (nth freqlst op)
                         ...
                         (ss/config! flab :text (format "%7.4f" (float (or f 0.0))))
                         (ss/config! blab :text (format "%4.2f" (float (or b 0.0))))
                         (ss/config! alab :text (format "%6.3f" (float (or a 0.0))))
                         (ss/config! jb :selected? (not (zero? e))))))
                   (.warning! ied "Internal Error: BUG 0005")))]
        {:pan-main pan-main
         :syncfn syncfn}))
