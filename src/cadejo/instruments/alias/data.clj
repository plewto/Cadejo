...
(save-program   1 "Test"
 (alias-program
  ...
  (sh       :rate 7.000 :src :div1   :bias 0.000 :scale 1.000)
  (matrix
            :a1 [:env1   1.000] :a2 [:con    1.000] 
            :b1 [:env2   1.000] :b2 [:con    1.000] 
            :c1 [:lfo1   0.010] :c2 [:cca    1.000] 
            ...
            :h1 [:div    1.000] :h2 [:con    1.000] )
  ;; OSCILLATORS
  (osc1-freq 0.500 :bias 0.000 :fm1 [:c      1.000 0.00] :fm2 [:f      0.000 0.00])
  (osc1-wave 1.000             :w1  [:a      1.000     ] :w2  [:f      0.000     ])
  (osc1-amp  +0 :pan -0.00     :am1 [:con    0.000 0.00] :am2 [:con    0.000 0.00])

  (osc2-freq 0.750 :bias 0.000 :fm1 [:c      1.000 0.00] :fm2 [:con    0.000 0.00])
  (osc2-wave 0.500             :w1  [:d      0.200     ] :w2  [:con    0.000     ])
  (osc2-amp  +0 :pan -0.00     :am1 [:con    0.000 0.00] :am2 [:con    0.000 0.00])

  (osc3-freq 1.125 :bias 0.000 :fm1 [:c      1.000 0.00] :fm2 [:con    0.000 0.00])
  (osc3-wave 1.000             :w1  [:a      1.000     ] :w2  [:con    0.000     ])
  (osc3-amp  +0 :pan -0.00     :am1 [:con    0.000 0.00] :am2 [:con    0.000 0.00])
  ...
  (clip    :gain 1.000 :mix -1.000 :clip [0.00 :con    0.00])
  (filter1 :gain 1.000 :freq 10000 :fm1  [:b           5000] :fm2 [:con        0]
           :mode 0.00              :res  [0.00 :con    0.00] :pan [-1.00 :con    +0.00])
  (fold    :gain 1.000 :mix -1.000 :clip [0.00 :con    0.00])
  (filter2 :gain 1.000 :freq 10000 :fm1  [:b           5000] :fm2 [:con        0]
                                   :res  [0.00 :con    0.00] :pan [+1.00 :con    +0.00])
  ;; EFX
  (dry       +0 :amp 0.100 :port-time 0.000  :cc7->volume 0.000)
  (pshifter -99 :ratio [1.000 :con    0.000] :rand 0.00 :spread 0.30)
  (flanger  -99 :lfo [0.250 0.000] :mod [:con    0.000] :fb -0.50 :xmix 0.25)
  (echo1    -99 :delay [0.250 :con    0.000] :fb +0.50 :damp 0.00 :gate [:con    0.00] :pan -0.75)
  (echo2    -99 :delay [0.125 :con    0.000] :fb +0.50 :damp 0.00 :gate [:con    0.00] :pan +0.75)))
