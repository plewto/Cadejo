...
(defn pp-alias 
  ([pnum pname data remarks]
     (with-out-str
       ...
       (pp-ringmod data)
       (printf "%s;; FILTERS\n" pad2)
       (pp-clipper 'clip data)
       (pp-filter 1 data)
       (pp-clipper 'fold data)
       (pp-filter 2 data)
       (printf "%s;; EFX\n" pad2)
       (pp-dry data)
       (pp-pshifter data)
       (pp-flanger data)
       (pp-echo 1 data)(print "\n")
       (pp-echo 2 data)(println "))")
       (println))))


;; (defn pp-dump [pnum pname data remarks]
;;   (let [params [:osc1-fm1-source
;;                 :osc1-fm2-source]
;;         sb (StringBuilder.)]
;;     (.append sb "\n---------------------------------------\n")
;;     (doseq [p params]
;;       (let [v (get data p)]
;;         (.append sb (format "[%-12s] %s\n" p v))))
;;     (.append sb "\n")
;;     (.toString sb)))
