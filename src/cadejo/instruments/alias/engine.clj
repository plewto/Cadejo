...
(defn create-performance [chanobj keymode main-out
  ...
  (.add-controller! chanobj ccc 0.0 1.0 0.0)
  (.add-controller! chanobj ccd 0.0 1.0 0.0)
  (.add-controller! chanobj cc-volume 0.0 1.0 1.0)
  (let [performance (cadejo.midi.performance/performance chanobj :Alias keymode)
        a-bus (control-bus)
        b-bus (control-bus)
        c-bus (control-bus)
        d-bus (control-bus)
...
(defn alias-mono
  ([scene chan]
     (alias-mono scene chan 0))
  ([scene chan main-out & {:keys [cca ccb ccc ccd cc-volume]
                           :or {cca 1
                                ccb 16
                                ccc 17
                                ccd 4
