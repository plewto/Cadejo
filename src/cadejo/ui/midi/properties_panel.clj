...
(defn- create-bend-controls [drw p0 ed]
  (let [root (.root drw)
        ...
        b-curve (curve-button drw :bend [(+ x0 93)(+ y0 40)]
                              :click-action (fn [b _]
                                              (let [c (second (msb/current-multistate-button-state b))]
                                                ...
                                                (.put-property! (.node ed) :bend-curve c))))
        ...
        range-callback (fn [dbar]
                         (let [r (int (math/str->float (.current-display dbar)))]
                           ...
                           (.put-property! (.node ed) :bend-range r)))
...
(defn midi-property-panel [ed]
  (let [x 10
        y 10
        drw (sgwr.elements.drawing/native-drawing drawing-width drawing-height)

        bend-subpan (create-bend-controls drw [(+ x 10) (+ y 0)] ed)
...
(def mpp (midi-property-panel (dummy-editor)))
